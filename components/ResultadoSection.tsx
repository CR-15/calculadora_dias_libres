...
export default function ResultadoSection({ resultado, error }: Props) {
  ...
  return (
    <section className="px-7 pt-6 pb-[26px] bg-gradient-to-b from-[#fbfcfe] to-[#f6f9fc] border-t border-line max-sm:px-5">
      ...
      {error && !resultado && (
        <p className="m-0 text-sm text-[#a23c28] font-semibold">{error}</p>
      )}

      {resultado && !error && (
        <>
          <div className="grid grid-cols-2 gap-[14px] mb-[18px] max-sm:grid-cols-1">
            <div className="border border-line rounded-[10px] p-[13px_15px] bg-white">
          ...
          <div className="flex items-center gap-[11px] p-[13px_16px] rounded-[10px] border border-line bg-white">
            <span
